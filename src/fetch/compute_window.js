...
const computeWindow = async ({ chainName, redis, BlockModel }) => {
  ...
  const getRuntimeWindow = (windowId) => {
    return RuntimeWindow.findOne({ windowId })
      .catch(e => {
        if (e.message === 'path exists') {
          $logger.warn('Index not found, retrying in 10s...')
  ...
  const doComputeWindow = async ({ id, previousWindow }) => {
    ...
    if (currentWindow) {
      ...
      await doProcessBlock({
        ...
        previousBlock: (previousBlockNumber > -1) ? (await getBlock(previousBlockNumber)) : null
      })
    } else {
      let startBlockNumber = 0
      if (id > 0) {
        startBlockNumber = (previousWindow || (await getRuntimeWindow(id - 1))).stopBlock + 1
      }
