...
const apply = program => {
  program
    ...
    .option('-l --parallel-blocks <blocks>', 'number of parallel fetching tasks', cliParseInt, 5)
    .action(({ phalaRpc, couchbaseEndpoint, parallelBlocks, redisEndpoint }) => {
      import('@/fetch')
        .then(({ default: start }) => start({ phalaRpc, couchbaseEndpoint, redisEndpoint, parallelBlocks }))
        .catch((...e) => {
          $logger.error(...e)
          process.exit(-1)
        })
